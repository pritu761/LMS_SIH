...
export function UserDetailModal({ user, onClose, onApprove, onSuspend }: UserDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md overflow-y-auto">
      <div className="relative my-8 w-full max-w-2xl rounded-3xl border border-slate-700 bg-slate-900/95 p-6 sm:p-8 shadow-2xl backdrop-blur-xl space-y-6">
        <button
          onClick={onClose}
        ...
        {profile.bio && (
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-slate-300 leading-relaxed rounded-xl bg-slate-950/50 p-3.5 border border-slate-800/80">
              {profile.bio}
            </p>
          </div>
        )}
            ...
            {user.competencies && user.competencies.length > 0 ? (
              user.competencies.map((comp) => (
                <div
                  ...
                  className="rounded-xl border border-slate-800 bg-slate-950/60 p-3 flex items-center justify-between"
                >
                  <div>
                    <span className="text-[10px] font-mono text-indigo-400 uppercase">
                      {comp.code}
                    </span>
                    <h5 className="text-xs font-semibold text-white">{comp.competencyName}</h5>
                  </div>
            ...
            <div className="space-y-2">
              {profile.qualifications && profile.qualifications.length > 0 ? (
                profile.qualifications.map((q, idx) => (
                  <div key={idx} className="rounded-xl bg-slate-950/60 border border-slate-800 p-2.5 text-xs">
                    <div className="font-semibold text-white">{q.degree}</div>
                    <div className="text-[11px] text-slate-400">{q.institution} ({q.year})</div>
                  </div>
                ))
              ) : (
                <div className="text-xs text-slate-400 italic">No formal records listed.</div>
              )}
            </div>
          </div>
              ...
              {profile.experience && profile.experience.length > 0 ? (
                profile.experience.map((exp, idx) => (
                  <div key={idx} className="rounded-xl bg-slate-950/60 border border-slate-800 p-2.5 text-xs">
                    ...
                    <div className="text-[11px] text-slate-400">{exp.company} ({exp.startYear} - {exp.endYear})</div>
                  </div>
                ))
              ) : (
                <div className="text-xs text-slate-400 italic">No prior experience listed.</div>
              )}
            </div>
          </div>
        </div>
