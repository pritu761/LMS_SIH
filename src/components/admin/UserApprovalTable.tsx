...
export function UserApprovalTable({ initialUsersList }: UserApprovalTableProps) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 rounded-2xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 px-3 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300 transition-colors disabled:opacity-50"
            ...
            {pendingCount > 0 && (
              <div className="flex items-center gap-2 rounded-2xl bg-amber-500/10 border border-amber-500/30 px-3.5 py-1.5 text-xs font-semibold text-amber-700 dark:text-amber-300 tabular-nums">
        ...
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-200 dark:border-slate-700/80 bg-white dark:bg-slate-950/70 pl-10 pr-4 py-2 text-sm text-slate-900 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
            />
          </div>
      ...
      {/* Main Data Table */}
      <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 backdrop-blur-xl overflow-hidden shadow-xl">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-600 dark:text-slate-300">
            <thead className="border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950/80 text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-400 font-bold">
              <tr>
                  ...
                  <tr key={u.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <div>
                          <div className="font-bold text-slate-900 dark:text-white text-sm">{u.profile.fullName}</div>
                    ...
                    {/* Designation & Org */}
                    <td className="px-6 py-4">
                      <div className="text-slate-900 dark:text-slate-200 font-medium">{u.profile.headline || 'General Professional'}</div>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800/80 hover:bg-slate-200 dark:hover:bg-slate-700 px-2.5 py-1.5 text-xs font-medium text-slate-900 dark:text-slate-200 transition-colors"
                        ...
                        {u.status === 'APPROVED' && u.role !== 'ADMIN' && (
                          <button
                            ...
                            disabled={isUpdating}
                            className="rounded-lg border border-rose-500/30 bg-rose-500/10 hover:bg-rose-500/20 px-2.5 py-1.5 text-xs font-medium text-rose-700 dark:text-rose-300 transition-colors"
                          >
                            Suspend
                          </button>
                        )}
                        ...
                        {u.status === 'SUSPENDED' && (
                          <button
                            onClick={() => handleUpdateStatus(u.id, 'APPROVED')}
                            disabled={isUpdating}
                            className="rounded-lg bg-emerald-600/20 border border-emerald-500/40 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-600/30 px-2.5 py-1.5 text-xs font-medium transition-colors"
                          >
                            Reactivate
                          </button>
                        )}
