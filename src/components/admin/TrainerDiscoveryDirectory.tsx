    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-slate-400">IMD & MoES Accredited Experts</span>
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-slate-700 bg-slate-950 pl-10 pr-4 py-2.5 text-xs text-slate-200 placeholder-slate-500 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
            />
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredTrainers.map((trainer) => {
          ...
          return (
            <div
              key={trainer.id}
              className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6 backdrop-blur-xl hover:border-emerald-500/40 transition-all duration-300 flex flex-col justify-between space-y-4 group hover:-translate-y-1 hover:shadow-elevation-1"
            >
                ...
                <div className="flex items-start gap-3.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5">
                      <h3 className="font-bold text-white text-sm truncate">{trainer.profile.fullName}</h3>
                      <span title="Verified MoES Faculty">
                        <ShieldCheck className="h-4 w-4 text-emerald-400 shrink-0" />
                      </span>
                    </div>
                    <p className="text-[11px] text-slate-400 truncate mt-0.5">{trainer.designation || trainer.profile.headline}</p>
                ...
                <div className="space-y-1.5 pt-1">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {trainer.competencies.map((c) => (
                      <span
                        key={c.competencyId}
                        className="rounded-lg bg-slate-950 px-2 py-1 text-[10px] font-medium text-indigo-300 border border-slate-800 flex items-center gap-1"
                      >
                        <span>{c.code}</span>
              ...
              <div className="pt-3 border-t border-slate-800 flex items-center justify-between gap-2">
                <button
                  ...
                  className="rounded-xl border border-slate-700 bg-slate-800/80 hover:bg-slate-700 px-3 py-1.5 text-xs font-semibold text-slate-200 transition-colors"
                >
                  View Dossier
                </button>
      ...
      {selectedTrainer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in-up">
          <div className="w-full max-w-2xl rounded-3xl border border-slate-700 bg-slate-900 p-6 sm:p-8 shadow-2xl space-y-5 animate-scale-in relative">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-lg font-bold text-white">{selectedTrainer.profile.fullName}</h3>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">{selectedTrainer.centre || selectedTrainer.profile.organization}</p>
                </div>
              </div>
            ...
            <div className="space-y-4 text-xs">
              <div className="space-y-1">
                <span className="font-bold text-slate-300 uppercase tracking-wider text-[10px]">Academic Bio</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1 bg-slate-950 p-3 rounded-xl border border-slate-800">
                  <span className="font-bold text-slate-300 uppercase tracking-wider text-[10px]">Qualifications</span>
                ...
                <div className="space-y-1 bg-slate-950 p-3 rounded-xl border border-slate-800">
                  <span className="font-bold text-slate-300 uppercase tracking-wider text-[10px]">Certifications</span>
                  <div className="space-y-1 mt-1">
