...
// UV Danger classification with accessible theme contrast
function getUvClassification(uv: number) {
  if (uv < 3) return { label: 'Low', badgeClass: 'bg-emerald-500/15 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 border-emerald-500/30' };
  if (uv < 6) return { label: 'Moderate', badgeClass: 'bg-yellow-500/15 dark:bg-yellow-500/20 text-yellow-800 dark:text-yellow-300 border-yellow-500/30' };
  if (uv < 8) return { label: 'High', badgeClass: 'bg-orange-500/15 dark:bg-orange-500/20 text-orange-800 dark:text-orange-300 border-orange-500/30' };
  if (uv < 11) return { label: 'Very High', badgeClass: 'bg-red-500/15 dark:bg-red-500/20 text-red-700 dark:text-red-300 border-red-500/30' };
  return { label: 'Extreme', badgeClass: 'bg-purple-500/15 dark:bg-purple-600/30 text-purple-700 dark:text-purple-200 border-purple-500/40 animate-pulse' };
}

// Accessible WMO condition badge styling for both light and dark modes
function getConditionBadgeClass(wmo: { severity?: string; category?: string }) {
  switch (wmo.severity) {
    case 'extreme':
      return 'bg-red-500/15 dark:bg-red-600/30 text-red-700 dark:text-red-200 border-red-500/40 animate-pulse';
    case 'warning':
      return 'bg-orange-500/15 dark:bg-orange-500/20 text-orange-800 dark:text-orange-300 border-orange-500/40';
    case 'watch':
      return 'bg-yellow-500/15 dark:bg-yellow-500/20 text-yellow-800 dark:text-yellow-300 border-yellow-500/40';
    case 'advisory':
      return 'bg-sky-500/15 dark:bg-sky-500/20 text-sky-800 dark:text-sky-300 border-sky-500/40';
    default:
      if (wmo.category === 'clear') {
        return 'bg-amber-500/15 dark:bg-amber-500/20 text-[#9a7224] dark:text-amber-300 border-amber-500/30';
      }
...
export const WeatherMetricsHud: React.FC<WeatherMetricsHudProps> = ({
  ...
  if (isLoading || !weatherData) {
    return (
      <div className={`p-4 bg-white dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl animate-pulse space-y-4 shadow-sm dark:shadow-none ${className}`}>
  ...
  return (
    <div className={`relative bg-white dark:bg-slate-900/85 backdrop-blur-xl border border-slate-200 dark:border-slate-800/90 rounded-2xl p-4 sm:p-5 shadow-xl dark:shadow-2xl text-slate-900 dark:text-slate-100 ${className}`}>
      ...
      {isFallback && (
        <div className="mb-3 px-3 py-1.5 bg-amber-500/15 border border-amber-500/30 rounded-xl flex items-center justify-between text-xs text-amber-700 dark:text-amber-300">
          <div className="flex items-center space-x-2">
            <Radio className="w-3.5 h-3.5 text-[#c59b48] animate-pulse" />
            <span>Procedural Telemetry Active (Offline Resilient Mode)</span>
          </div>
          {lastUpdated && <span className="text-[10px] text-amber-600 dark:text-amber-400/80 font-mono">{lastUpdated}</span>}
        </div>
      )}
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800/80 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 hover:border-[#c59b48]/50 text-slate-600 dark:text-slate-300 hover:text-[#9a7224] dark:hover:text-amber-300 transition-colors"
      ...
      {showSettings && (
        <div className="mb-4 p-3 bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-xl space-y-2 text-xs animate-fadeIn">
          <div className="flex items-center justify-between">
            <span className="text-slate-600 dark:text-slate-400 font-medium">Temperature:</span>
            <div className="flex rounded-lg bg-slate-200/80 dark:bg-slate-900 p-0.5 border border-slate-300 dark:border-slate-800">
              <button
                type="button"
          ...
          <div className="flex items-center justify-between">
            <span className="text-slate-600 dark:text-slate-400 font-medium">Wind Speed:</span>
            <div className="flex rounded-lg bg-slate-200/80 dark:bg-slate-900 p-0.5 border border-slate-300 dark:border-slate-800">
          ...
          <div className="flex items-center justify-between">
            <span className="text-slate-600 dark:text-slate-400 font-medium">Barometric Pressure:</span>
            <div className="flex rounded-lg bg-slate-200/80 dark:bg-slate-900 p-0.5 border border-slate-300 dark:border-slate-800">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800/80 gap-4">
        <div className="flex items-center space-x-4">
          <div className="p-3.5 bg-slate-100 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 rounded-2xl shadow-inner flex items-center justify-center">
          ...
          <div>
            <div className="flex items-baseline space-x-1">
              ...
              <span className="text-2xl text-[#9a7224] dark:text-amber-400 font-semibold">{tempUnitSymbol}</span>
            </div>
            <div className="flex items-center space-x-2 text-xs text-slate-600 dark:text-slate-300 mt-0.5">
              <span>Feels like <strong className="text-slate-900 dark:text-white">{feelsLikeFormatted}{tempUnitSymbol}</strong></span>
              <span>•</span>
              <div className="flex items-center space-x-1 text-slate-500 dark:text-slate-400">
                <ArrowUp className="w-3 h-3 text-red-500 dark:text-red-400" />
        ...
        <div className="flex flex-col sm:items-end space-y-1.5">
          <div className={`px-3 py-1.5 rounded-xl border text-xs font-semibold inline-flex items-center space-x-2 ${getConditionBadgeClass(wmo)}`}>
            <span>{wmo.label}</span>
          </div>
          <div className="flex items-center space-x-2 text-xs text-slate-500 dark:text-slate-400">
            <span className="flex items-center space-x-1 px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800/90 border border-slate-200 dark:border-slate-700 text-[11px] text-slate-700 dark:text-slate-300">
              <Radio className="w-3.5 h-3.5 text-[#c59b48]" />
              <span>Est. Radar: <strong className="text-[#9a7224] dark:text-amber-300 font-mono">{derivedDbz} dBZ</strong></span>
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 pt-4">
        ...
        <div className="p-3 bg-slate-50/80 hover:bg-slate-100 dark:bg-slate-800/40 dark:hover:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/50 rounded-xl transition-all">
          ...
          <div className="text-lg font-bold text-slate-900 dark:text-white font-mono">
            {windSpeedFormatted} <span className="text-xs font-normal text-slate-500 dark:text-slate-400">{windUnitSymbol}</span>
          </div>
          <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-1 truncate">
            Gusts: <strong className="text-slate-800 dark:text-slate-200">{windGustsFormatted} {windUnitSymbol}</strong>
          </div>
        </div>
        ...
        <div className="p-3 bg-slate-50/80 hover:bg-slate-100 dark:bg-slate-800/40 dark:hover:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/50 rounded-xl transition-all">
          ...
          <div className="text-lg font-bold text-slate-900 dark:text-white font-mono">{current.uvIndex} <span className="text-xs text-slate-400 dark:text-slate-500 font-normal">/ 12</span></div>
          <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 rounded-full mt-2 overflow-hidden">
            <div
        ...
        <div className="p-3 bg-slate-50/80 hover:bg-slate-100 dark:bg-slate-800/40 dark:hover:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/50 rounded-xl transition-all">
          ...
          <div className="text-lg font-bold text-slate-900 dark:text-white font-mono">{current.cloudCover}%</div>
          <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 rounded-full mt-2 overflow-hidden">
            <div
