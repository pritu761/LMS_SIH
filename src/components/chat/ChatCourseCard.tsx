...
export function ChatCourseCard({ course, onNavigate }: ChatCourseCardProps) {
  const cadreColors: Record<string, { bg: string; text: string; border: string }> = {
    DRSTC: { bg: 'bg-[#c59b48]/15', text: 'text-[#dfb76c]', border: 'border-[#c59b48]/30' },
  ...
  return (
    <div className="my-2 group relative overflow-hidden rounded-2xl border border-white/10 bg-white dark:bg-slate-900/90 p-3.5 backdrop-blur-md transition-all duration-300 hover:border-[#c59b48]/50 hover:bg-slate-800/90 shadow-lg hover:shadow-[#c59b48]/15">
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <Link href={`/trainee/courses/${course.id}`} onClick={onNavigate} className="block">
            <h4 className="text-xs font-bold text-slate-900 dark:text-white group-hover:text-[#c59b48] transition-colors line-clamp-1 hover:underline cursor-pointer">
              {course.title}
            </h4>
          </Link>
