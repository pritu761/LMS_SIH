...
export function TraineeSkillGapCard({ userId }: Props) {
  ...
  return (
    <div className="rounded-3xl border border-[#c59b48]/30 bg-gradient-to-br from-[#0b1e36]/90 via-[#102744] to-[#081526] p-6 sm:p-8 backdrop-blur-xl shadow-elevation-2 space-y-6 relative overflow-hidden">
      {/* Top accent glow */}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="rounded-md bg-[#c59b48]/15 px-2.5 py-0.5 text-[10px] font-black text-[#dfb76c] border border-[#c59b48]/30 uppercase tracking-wider">
              CADRE BENCHMARK TRACKING
            </span>
            <span className="text-xs text-slate-300 font-mono">Mission Mausam Protocol</span>
          </div>
          ...
          <p className="text-xs text-slate-300 mt-1">
            Track your verified skills against <span className="text-[#dfb76c] font-semibold">{analysis.cadreBenchmarkName}</span> requirements.
          </p>
        </div>
        ...
        <div className="shrink-0 space-y-1">
          ...
          <select
            ...
            className="rounded-2xl border border-slate-700 bg-slate-950 px-3.5 py-1.5 text-xs font-bold text-white focus:border-[#c59b48] focus:outline-none shadow-md"
          >
            {initialCadres.map((c) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {deficientSkills.slice(0, 2).map((item) => (
            <div
              ...
              className="rounded-2xl border border-white/10 bg-slate-950/70 p-4 space-y-3 hover:border-[#c59b48]/60 transition-all shadow-lg hover:shadow-[#c59b48]/10"
            >
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-[10px] font-mono text-[#dfb76c] font-bold">{item.code}</span>
