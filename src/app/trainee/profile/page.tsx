  ...
  return (
    <div className="flex-1 flex max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 gap-6">
      ...
      <main className="flex-1 min-w-0 space-y-6">
        ...
        {/* Header */}
        <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-6 sm:p-8 backdrop-blur-xl space-y-2 shadow-sm dark:shadow-none">
          <div className="flex items-center gap-2">
            <span className="rounded-md bg-indigo-500/10 px-2.5 py-0.5 text-xs font-bold text-indigo-700 dark:text-indigo-400 border border-indigo-500/20">
              MISSION MAUSAM DOSSIER
            </span>
            <span className="text-xs text-slate-500 dark:text-slate-400">IMD / MoES Official Cadre Profile</span>
          </div>
        ...
        {isSaved && (
          <div className="rounded-2xl bg-emerald-500/10 border border-emerald-500/30 p-4 text-xs font-semibold text-emerald-800 dark:text-emerald-300 flex items-center gap-2">
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-6 backdrop-blur-xl space-y-4 shadow-sm dark:shadow-none">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Official Full Name</label>
                <input
                  type="text"
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Cadre Training Track</label>
                <select
                  value={cadreTrack}
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Official Designation</label>
                <input
                  type="text"
                  value={designation}
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Station / Division / RMC</label>
                <input
                  type="text"
                  value={centre}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Professional Research & Training Bio</label>
              <textarea
                rows={3}
                value={profile.bio}
          ...
          <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-6 backdrop-blur-xl space-y-4 shadow-sm dark:shadow-none">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] text-slate-500 dark:text-slate-400">1: Foundational • 5: Master Practitioner</span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {initialCompetencies.map((comp) => {
                ...
                return (
                  <div
                    key={comp.id}
                    className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/60 p-4 space-y-3 shadow-sm dark:shadow-none"
                  >
                    <div className="flex items-center justify-between">
                      <div>
                      ...
                      <span className="rounded bg-indigo-50 dark:bg-indigo-600/20 px-2 py-0.5 text-xs font-bold text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-500/30">
                        Level {currentLevel}
                      </span>
                    </div>
          ...
          <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-6 backdrop-blur-xl space-y-4 shadow-sm dark:shadow-none">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="flex items-center gap-1 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 px-3 py-1.5 text-xs font-semibold text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 transition-colors shadow-sm dark:shadow-none"
              >
            ...
            <div className="space-y-3">
              {qualifications.map((q, idx) => (
                <div
                  key={idx}
                  className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/60 p-4 flex flex-col sm:flex-row items-start sm:items-center gap-3 shadow-sm dark:shadow-none"
                >
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 flex-1 w-full">
                    <input
                      ...
                      className="rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-2.5 py-1.5 text-xs text-slate-900 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 shadow-sm dark:shadow-none"
                    />
                    <input
                      type="text"
